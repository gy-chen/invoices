...
class PrizesParser:
    ...
    _pattern_date = re.compile(r'(\d{3})年(\d{2})-(\d{2})月' if six.PY3 else ur'(\d{3})年(\d{2})-(\d{2})月' )
    # pattern for searching prize numbers of the prizes page
    _pattern_prizes = re.compile(
        '(\\d{8}).*?(\\d{8}).*?(\\d{8})、(\\d{8})、(\\d{8}).*?(\\d{3})、?(\\d{3})?、?(\\d{3})?'
        if six.PY3 else
        u'(\\d{8}).*?(\\d{8}).*?(\\d{8})、(\\d{8})、(\\d{8}).*?(\\d{3})、?(\\d{3})?、?(\\d{3})?',
        re.DOTALL)

    def __init__(self, page_content):
        self._soup = BeautifulSoup(page_content, 'html5lib')
